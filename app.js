...
function readHash() {
  ...
  const pipes = sPipes.map((sTok, j) => {
    ...
    if (sections.length >= 1) {
      ...
      sections[0].endA = decodeEnd(ends[0]);
      sections[sections.length - 1].endB = decodeEnd(ends[1]);
    }
...
function writeHash(params) {
  ...
  const endPair = (pp) => encodeEnd(pp.sections[0].endA) + '|' + encodeEnd(pp.sections[pp.sections.length - 1].endB);
  ...
  if (pipes.length === 1) {
    ...
    s += '&e0=' + encodeEnd(secs[0].endA) + '&eN=' + encodeEnd(secs[secs.length - 1].endB);
  } else {
    s += '&e=' + pipes.map(endPair).join(';');
  }
...
//   p<j>.s<i>.endA.type | p<j>.s<i>.endA.<suf>      left end treatment (first section)
//   p<j>.s<i>.endB.type | p<j>.s<i>.endB.<suf>      right end treatment (last section)
...
function parseKey(key) {
  ...
  let m;
  if ((m = /^s(\d+)\.(endA|endB)\.(.+)$/.exec(rest))) return { pi, kind: 'end', i: +m[1], slot: m[2], leaf: m[3] };
...
function getP(params, key) {
  ...
  const end = pp.sections[k.i] && pp.sections[k.i][k.slot];
...
function setLeaf(params, key, v) {
  ...
  else pp.sections[k.i][k.slot][k.leaf] = v;
}
...
// two end treatments onto its geometric extremes (endA on the first section,
// endB on the last - the same section when only one is left). A section added
// at a mated end therefore inherits the joint - the treatment follows the
// extreme, and syncJoints re-locks the dimensions.
...
function structuralEdit(pi, mutate, driver) {
  const p = geo.cloneParams(state.params);
  const pp = p.pipes[pi];
  const endA = pp.sections[0].endA;
  const endB = pp.sections[pp.sections.length - 1].endB;
  mutate(pp);
...
// end treatments onto the new extremes - which is exactly what makes this work
// at a mated end: the new section takes over the joint, inheriting the
...
function removeSection(pi, i) {
  ...
  if (pipe.sections.length <= 1) return;            // a pipe is at least one straight run
...
// End-treatment controls for one open end of section `i` of pipe `pi` (which
// must be a first/last section). `slot` is 'endA' (the left end, on the first
// section) or 'endB' (the right end, on the last). Only a section that is both
// first and last shows one block of each, and there a "Left end" / "Right end"
// heading tells the two apart - the controls themselves are labelled the same
// either way. `mate` is null for a free end, else { pipe } naming the pipe on
// the other side of the joint - which restricts the menu and labels the shared
// values, so it is visible which numbers belong to both parts.
function endControls(pi, i, slot, mate) {
  const sec = state.params.pipes[pi].sections[i];
  const end = sec[slot];
  const side = slot === 'endA' ? 'Left' : 'Right';
  const pre = 'p' + pi + '.s' + i + '.' + slot + '.';
...
function groupModel(chain) {
  ...
  for (let pi = 0; pi < nPipes; pi++) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      // and the very section that faces this one across it. Each of the two end
      // slots has its own mate — a lone section in a middle pipe answers to a
      // joint on each side.
      const mateOf = (j, side) => {
        const secs = pipes[j].sections;
        return { pipe: j, section: side === 'l' ? secs[secs.length - 1] : secs[0] };
      };
      const mateA = isFirst && !firstPipe ? mateOf(pi - 1, 'l') : null;
      const mateB = isLast && !lastPipe ? mateOf(pi + 1, 'r') : null;
      const mate = mateA || mateB;
      const spre = 'p' + pi + '.s' + i + '.';
      ...
      // The section's own dimensions answer to every joint it touches, which for
      // a lone section between two pipes is both of them.
      const mates = [mateA, mateB].filter(Boolean);
      const shared = mates.length
        ? ' (shared with Pipe ' + mates.map((m) => m.pipe + 1).join(' & Pipe ') + ')' : '';
      const controls = [
        ...
        // The pipe's two open ends: the first section owns the left one, the
        // last the right. A lone section owns both and shows the two blocks in
        // order, each with its own mate (or none).
        ...(isFirst ? endControls(pi, i, 'endA', mateA) : []),
        ...(isLast ? endControls(pi, i, 'endB', mateB) : []),
      ];
      // Only the two end sections can add or remove: the first prepends a new
      // first section, the last appends a new last section. (A lone section is
      // both; its + Section appends.)
      groups.push({
        id: gid(pi, 's', i), kind: 'section', index: i, pipe: pi,
        tag: String(i + 1), title: 'Section ' + (i + 1),
        mate: !!mate,
        onAdd: isLast ? () => addSectionAfter(pi, i) : isFirst ? () => addSectionBefore(pi, 0) : null,
        addTitle: isLast ? 'Add a new last section (copies this one)' : 'Add a new first section (copies this one)',
        onRemove: ((isFirst || isLast) && N > 1) ? () => removeSection(pi, i) : null,
...
function render() {
  ...
  if (g) {
    ...
    const sig = [
      ...
      pipes.map((pp) => [
        pp.sections.length,
        endSig(pp.sections[0].endA), endSig(pp.sections[pp.sections.length - 1].endB),
