...
// wall of material, and at the section length, so it never outgrows the section
// it sits in.
const fitFloor = (wall, engage, secLen) => Math.min(wall, engage, secLen);

// The axial length a slip joint adds to its section: exactly its engagement.
const fitLen = (sec) => {
...
function fitEnd(sec, index, notes) {
  ...
  if (e.type === 'chamfer') {
    ...
    // the section's length, so nothing here has to give.
    //
...
// The mating section of a pipe on the given side of a joint ('l' = the pipe to
// the left of the joint, so its last section; 'r' = the pipe to the right).
const mateSection = (pipe, side) => (side === 'l' ? pipe.sections[pipe.sections.length - 1] : pipe.sections[0]);
...
function jointStub(pipeL, pipeR) {
  const a = mateSection(pipeL, 'l'), b = mateSection(pipeR, 'r');
  if (a.end && a.end.type === 'fit') return fitLen(a);
  if (b.end && b.end.type === 'fit') return fitLen(b);
  return 0;
}
...
function syncJoint(src, dst) {
  ...
  if (!mateableEnds(src.end.type, dst.end.type)) dst.end.type = MATE_OF[src.end.type] || 'plain';
  for (const k of (MATE_END_KEYS[src.end.type] || [])) {
    if (dst.end.type === src.end.type) dst.end[k] = src.end[k];
  }
}
...
// is simply the mating section's length. Only the inside (male) side is bounded
// this way: a socket receives the mate instead of entering it, so nothing over
// there limits how deep it can be. With no pipe joined on there is nothing to
// enter, and the joint length stays free.
//
...
function capSpigot(pipes, j) {
  const cap = (sec, mate) => {
    ...
    if (e.FitL > mate.l) e.FitL = round(mate.l, 2);
  };
  const a = mateSection(pipes[j], 'l'), b = mateSection(pipes[j + 1], 'r');
  cap(a, b);
  cap(b, a);
}
...
// of the edited side; every other joint flows left to right. A pipe's first and
// last sections are always distinct (two-section minimum), so one pass settles
// the whole chain - no joint can feed another.
function syncJoints(pipes, driver) {
  for (let j = 0; j < pipes.length - 1; j++) {
    const L = pipes[j], R = pipes[j + 1];
    const fromRight = !!driver && driver.pi === j + 1 && driver.si === 0;
    const a = mateSection(L, 'l'), b = mateSection(R, 'r');
...
function endFeature(end, baseOuter, sec) {
  ...
  if (type === 'chamfer') {
    ...
    const E = end.FitL, floor = fitFloor(st.w, E, sec.l);
    if (st.side === 'inside') {
...
function innerEndFeature(end, baseInner, sec) {
  if (end.type === 'fit' && end.FitL > 0) {
    ...
    if (st.side === 'inside') {
      ...
      return [{ d: 0, r: st.sI }, { d: E, r: st.sI }, { d: sec.l + E, r: st.I }];
    }
